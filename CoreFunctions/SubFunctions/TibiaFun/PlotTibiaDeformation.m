...
for alt=Alt
    ...
    [~,Imax] = max(Curve(1).Pts(:,1));
%     Imax
%     Curve(1).Pts(Imax,:)
    FrontEdgePts(i,1:3) = Curve(1).Pts(Imax,:);
end


FrontEdgePts = ProjectOnPlan(FrontEdgePts,[0;1;0],zeros(1,3));

[V,~] = eig(cov(FrontEdgePts));

UEdge = V(:,3);

CenterEdgeFront = mean(FrontEdgePts);
...
plotCylinder( [0; 0; -1], 0.75, [0, 0, 0.5*min(DistTib.Points(:,3)) + 0.5*max(ProxTib.Points(:,3))], abs(min(DistTib.Points(:,3))) + 20, 1, 'k')
plotCylinder( UEdge, 0.75, CenterEdgeFront, 180, 1, 'b')
view(180,0)
