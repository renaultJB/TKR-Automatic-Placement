function [ Tstring , T_str_anat , PC_ML_Width , PC_AP_Width] = PositionProth2( name, alpha, Right_Knee , Generate_Pos )
...
if YPostAnt_0'*Yprost<0
    Yprost = - Yprost;
    warning('Yprost was flipped and is Ranat is no longer a direct orthonormed coordinate system')
end
...
d_xp = - ( Centroid_TP_onPlan - (max(distML,distAP)+3)*Zprost')*Zprost;
...
Start_Point = Centroid_pc-Yprost'*0.36*PC_AP_Width+0.02*LegSide*Xprost'*PC_ML_Width...
    +7*Zprost'; %Prosthesis thickness 5.5 +1.5 cement thickness      +LegSide*alpha*Vpc(:,1)'
%
StemTip = Start_Point - 146*Zprost' + 19.6*Yprost';

PtsStemTip = Slice_pop(Points,Zmech,-StemTip*Zmech,1.5); % Points of diaphysis stem tips
CDiaphysisStemTip_CT = mean(PtsStemTip) ;
...
f = @(x)CenteringStemTipGoal(x , ProsthOrig0 , CDiaphysisStemTip );  % C or CDiaphysisStemTip
...
XYZ = py.txt2mtlb.read_nodesGMSH('data\Cement2_S4_WLS.msh');
Pts_prosthesis = [cell2mat(cell(XYZ{'X'}))' cell2mat(cell(XYZ{'Y'}))' cell2mat(cell(XYZ{'Z'}))'];
